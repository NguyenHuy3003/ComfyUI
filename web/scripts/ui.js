...
export class ComfyUI {
	constructor(app) {
		...
		async function getWorkflowAsJSON() {
			try {
				 let res =  await app.graphToPrompt().then(p=>{
					const json = p.output; 
					return json
				})
				return res
		...
		this.menuContainer = $el("div.comfy-menu", {parent: document.body}, [
			...
			$el("button", {
				id: "node-save-workflow",
				textContent: "Publish",
				onclick: async () => {
				  const createInputElement = (type, placeholder, id) => {
				  ...
				  const inputInfo = [
					{ id: 'nameWorkflow', placeholder: 'Name Workflow', type: 'text' },
					{ id: 'descriptionWorkflow', placeholder: 'Description Workflow', type: 'text' },
				  ];
			  
				  inputInfo.forEach(info => {
					const input = createInputElement(info.type, info.placeholder, info.id);
				  ...
				  dialog.appendChild(document.createTextNode(' ')); // Khoảng cách
			  
				  const sendDataButton = document.createElement('button');
				  sendDataButton.textContent = 'Save';
				  sendDataButton.onclick = async () => {
					const nameInput = document.getElementById('nameWorkflow').value;
					const descInput = document.getElementById('descriptionWorkflow').value;
			  
					
					  try {
						let json = await getWorkflowAsJSON()
						const response = await fetch('http://127.0.0.1:8181/api/workflow', {
						  method: 'POST',
						  headers: {
							'Content-Type': 'application/json'
						  },

							 body: JSON.stringify({ workflow: { nameWorkflow: nameInput, descriptionWorkflow: descInput, jsonWorkflow: json } })
						});
						const responseData = await response.json();
			  
						if (responseData.message === false) {
							errorDiv.textContent = 'Name WorkFlow is existing, pl changle ';
						} else {
							errorDiv.textContent = 'Data save successfully ';

						}
					  } catch (error) {
						console.error('Error:', error);
						errorDiv.textContent = 'Error save to DB ';
					  }
			  
					
					
				  };
				  dialog.appendChild(sendDataButton);
			  
				  dialog.appendChild(document.createTextNode(' ')); // Khoảng cách
			  
				  const closeButtonAll = document.createElement('button');
				  closeButtonAll.textContent = 'Close';
				  closeButtonAll.onclick = () => {
					document.body.removeChild(dialog);
				  };
				  dialog.appendChild(closeButtonAll);
